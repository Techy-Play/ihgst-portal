...
// ─── Main engine ───────────────────────────────────────────────────────────
export async function runEscalationEngine({ triggeredBy = 'cron', triggeredByName = 'System' } = {}) {
  await dbConnect();
  ...
  try {
    ...
    if (!activeCycles.length) {
      await CronLog.create({ triggeredBy, triggeredByName, status: 'success', created: 0, resolved: 0, durationMs: Date.now() - startTime, message: 'No active cycles found. Engine skipped.' });
      return results;
    }

    // Pre-fetch all employees and admins once
    ...
    for (const cycle of activeCycles) {
      ...
      if (!cycle.goalSettingStart) continue; // Guard: cycle must have a start date
      ...
      for (const employee of allEmployees) {
        try {
          ...
          if (created) {
            ...
            // Always notify employee (all levels)
            ...
            await tryEmail(employee.email, `[IHGST] Action Required: Submit Your Goals for ${cycle.name}`,
              `<p>Hi ${employee.name},</p><p>Your goals for <strong>${cycle.name}</strong> are still in draft. Please log in and submit them before the deadline.</p><p><a href="${process.env.NEXTAUTH_URL}/goals">Go to My Goals →</a></p>`);

            // L2 → also notify & email manager
            if (level === 'LEVEL_2' && employee.managerId) {
              ...
              if (manager?.email) {
                await tryEmail(manager.email, `[IHGST] Escalation: ${employee.name}'s Goals Overdue`,
                  `<p>Hi ${manager.name},</p><p><strong>${employee.name}</strong> has not submitted goals for <strong>${cycle.name}</strong> and is currently ${Math.floor(daysSinceCycleOpen)} days overdue. Please follow up.</p><p><a href="${process.env.NEXTAUTH_URL}/manager">View Team Goals →</a></p>`);
              }
            }

            // L3 → also notify & email all admins
            if (level === 'LEVEL_3') {
              for (const admin of admins) {
                ...
                if (admin.email) {
                  await tryEmail(admin.email, `[IHGST] Critical Escalation: ${employee.name}'s Goals Not Submitted`,
                    `<p>Hi ${admin.name},</p><p><strong>${employee.name}</strong> has failed to submit their goals for <strong>${cycle.name}</strong> and is now ${Math.floor(daysSinceCycleOpen)} days overdue. Immediate action is required.</p><p><a href="${process.env.NEXTAUTH_URL}/admin/escalations">View Escalations Dashboard →</a></p>`);
                }
              }
            }
            ...
            await AuditLog.create({
              ...
              action: 'escalation_created', changedByName: triggeredByName,
            ...
            if (employee.managerId) {
              ...
              if (manager?.email) {
                await tryEmail(manager.email, `[IHGST] Approval Overdue: ${employee.name}'s Goals`,
                  `<p>Hi ${manager.name},</p><p>${employee.name}'s goal sheet for <strong>${cycle.name}</strong> has been pending approval for <strong>${Math.floor(daysSinceSubmit)} days</strong>. Please review it.</p><p><a href="${process.env.NEXTAUTH_URL}/manager">Review Now →</a></p>`);
              }
            }

            // L2 → also notify & email admins
            if (level === 'LEVEL_2') {
              for (const admin of admins) {
                ...
                if (admin.email) {
                  await tryEmail(admin.email, `[IHGST] Escalation: ${employee.name}'s Goal Approval Overdue`,
                    `<p>Hi ${admin.name},</p><p><strong>${employee.name}'s</strong> goals for <strong>${cycle.name}</strong> have been pending manager approval for ${Math.floor(daysSinceSubmit)} days. Please review the bottleneck.</p><p><a href="${process.env.NEXTAUTH_URL}/admin/escalations">View Escalations →</a></p>`);
                }
              }
            }
        ...
        try {
          ...
          if (created) {
            ...
            await tryEmail(employee.email, `[IHGST] ${activeQuarter} Check-in Required — ${cycle.name}`,
              `<p>Hi ${employee.name},</p><p>Your <strong>${activeQuarter} check-in</strong> for <strong>${cycle.name}</strong> is overdue. Please log in and submit your progress.</p><p><a href="${process.env.NEXTAUTH_URL}/checkin">Submit Check-in →</a></p>`);
            ...
            if (level === 'LEVEL_2' && employee.managerId) {
              ...
              if (manager?.email) {
                await tryEmail(manager.email, `[IHGST] Escalation: ${employee.name}'s Check-in Overdue`,
                  `<p>Hi ${manager.name},</p><p><strong>${employee.name}</strong> has not submitted their <strong>${activeQuarter}</strong> check-in for <strong>${cycle.name}</strong>. Please follow up with them.</p><p><a href="${process.env.NEXTAUTH_URL}/manager">View Team →</a></p>`);
              }
            }
    ...
    // Log the fatal error and rethrow
    await CronLog.create({
      triggeredBy, triggeredByName, status: 'error',
      created: results.created, resolved: results.resolved,
      errorDetails: [fatalErr.message],
      durationMs: Date.now() - startTime,
      message: `Fatal error: ${fatalErr.message}`,
    }).catch(() => {});
    throw fatalErr;
  }
  ...
  await CronLog.create({
    ...
    durationMs,
    message: `${results.created} created, ${results.resolved} resolved${results.errors.length ? `, ${results.errors.length} errors` : ''} in ${durationMs}ms`,
  }).catch(() => {});

  return results;
}

// ─── Route handler (Vercel Cron) ────────────────────────────────────────────
export async function GET(request) {
  // Validate cron secret
  const authHeader = request.headers.get('authorization');
  const expectedSecret = process.env.CRON_SECRET;

  // If CRON_SECRET is set, enforce it. If not set, allow only Vercel's internal calls.
  if (expectedSecret) {
    if (authHeader !== `Bearer ${expectedSecret}`) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }
  } else {
    // No secret configured — still check for Vercel's cron header to block public calls
    const vercelEnv = request.headers.get('x-vercel-cron');
    if (!vercelEnv) {
      return NextResponse.json({ error: 'Set CRON_SECRET in environment variables to protect this endpoint' }, { status: 401 });
    }
  }

  try {
    const results = await runEscalationEngine({ triggeredBy: 'cron', triggeredByName: 'Vercel Cron' });
