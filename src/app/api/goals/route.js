...
export async function GET(request) {
  try {
    ...
    let query = { userId };
    if (cycleId) {
      const goalSheet = await GoalSheet.findOne({ userId, cycleId });
      if (goalSheet) query.goalSheetId = goalSheet._id;
    }
    ...
    if (goals.length > 0) {
      ...
      if (activeCycle) goalSheet = await GoalSheet.findOne({ userId, cycleId: activeCycle._id }).lean();
    }

    const totalWeightage = goals.reduce((sum, g) => sum + (g.weightage || 0), 0);
    return NextResponse.json({ goals, goalSheet, totalWeightage });
  } catch (error) {
...
export async function POST(request) {
  try {
    ...
    const goal = await Goal.create({
      userId: session.user.id, goalSheetId: goalSheet._id,
    ...
    await AuditLog.create({ entityType: 'Goal', entityId: goal._id, action: 'created', changedBy: session.user.id, changedByName: session.user.name, description: `Goal "${goal.title}" created` });
