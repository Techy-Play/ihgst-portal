...
function UnauthorizedContent() {
  ...
  const attemptedPath = searchParams.get('from') || '/unknown';
  ...
  return (
    <div style={{ width: '100%', height: '100vh', background: '#000', overflowX: 'hidden', display: 'flex', justifyContent: 'center', alignItems: 'center', position: 'relative' }}>
      <MessageDisplay router={router} attemptedPath={attemptedPath} />
      <CharactersAnimation />
      <CircleAnimation />
    </div>
  );
  ...
  return (
    <Suspense fallback={
      <div style={{ width: '100%', height: '100vh', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: 40, height: 40, border: '3px solid #333', borderTopColor: '#818cf8', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
      </div>
    }>
      <UnauthorizedContent />
    </Suspense>
...
function MessageDisplay({ router, attemptedPath }) {
    ...
    <div style={{ position: 'absolute', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', width: '90%', height: '90%', zIndex: 100 }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', transition: 'opacity 0.5s', opacity: isVisible ? 1 : 0 }}>
        ...
        <div style={{ fontSize: '15px', width: '50%', minWidth: '40%', textAlign: 'center', color: '#000', margin: '1%' }}>
          You don&apos;t have permission to access this page. Your current role doesn&apos;t allow viewing this resource.
        </div>
        <div style={{ fontSize: '12px', color: '#555', margin: '8px 0 0', fontFamily: 'monospace', background: 'rgba(0,0,0,0.06)', padding: '4px 12px', borderRadius: '6px' }}>
